...
class GetFavoritePlantsUseCase(private val plantRepository: PlantRepository) : NoParamUseCase<List<Plant>>() {
    override suspend fun execute(): Result<List<Plant>> {
        return plantRepository.getFavoritePlants().first()
    }

    fun getFlow(): Flow<Result<List<Plant>>> {
        return plantRepository.getFavoritePlants()
    }
}